...
const Skill = () => {
  return (
    <div className="max-w-7xl  mx-auto text-center mt-28">
      <h1 className="text-4xl font-bold mb-10 text-white dark:text-gray-600">
        Skills{" "}
        <span className="text-[#10E956] dark:text-gray-600">Front End</span>
      </h1>
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-10 justify-center p-4">
        {skills.map((skill, index) => (
          <div
            key={index}
            className="flex flex-col items-center shadow-2xl rounded-lg p-6 dark:bg-[#0A0A0A] 
                       transform transition-all duration-300 ease-in-out hover:scale-105 dark:hover:bg-[#222] hover:shadow-2xl"
          >
            <img
              src={skill.img}
              alt={skill.title}
              className="h-24 w-24 lg:dark:filter lg:dark:grayscale lg:dark:hover:grayscale-0 transition-all duration-300 ease-in-out rounded-lg mb-4"
            />
      ...
      <h1 className="text-4xl font-bold mb-10 text-white dark:text-gray-600">
        Skills <span className="text-[#10E956] dark:text-gray-600"> Others</span>
      </h1>
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 m-4 gap-10 justify-center">
        {otherSkills.map((skill, index) => (
          <div
            key={index}
            className="flex flex-col items-center shadow-2xl rounded-lg p-6 dark:bg-[#0A0A0A] 
                       transform transition-all duration-300 ease-in  hover:scale-105 dark:hover:bg-[#222] hover:shadow-2xl"
          >
            <img
              src={skill.img}
              alt={skill.title}
              className="h-24 w-24 rounded-lg  lg:dark:filter lg:dark:grayscale lg:dark:hover:grayscale-0transition-all duration-300 ease-in-out mb-4"
            />
            <h2 className="text-xl font-semibold text-gray-600">{skill.title}</h2>
          </div>
        ))}
      </div>
    </div>
